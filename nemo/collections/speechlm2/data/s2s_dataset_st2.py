...
class DuplexS2SDatasetST(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.video_pad_token_id = self.tokenizer.text_to_ids("<|video_pad|>")[0]
...
def _pad_chunk(n: int, pad_id: int, dtype=torch.long):
    if n <= 0:
        return torch.empty(0, dtype=dtype)
    return torch.full((n,), pad_id, dtype=dtype)
...
def build_token_channel(
    ...
    video_pad_id = tokenizer.text_to_ids("<|video_pad|>")[0] # temporary id for speech
    ...
    # total = src + tgt + len(src_alignments) * 2 # to ensure sufficient buffer space 
   # tokens = torch.ones(total, dtype=torch.long) * pad_id
   # both source and target trajectories are required to have the same length
    ...
    for i, (src_alignment, tgt_alignment) in enumerate(zip(src_alignments, tgt_alignments)):
        ...
        if src_alignment and tgt_alignment:
            # if both source and target alignment for the chunk are available update offset and add tgt text after src speech
        ...
        # this depends if we later want the target audio conditionally dependent on the text
            ...
            # if only src speech is available, update offset 
            src_alig_frames = [src_alignment[1], src_alignment[1]+src_alignment[2]]
            src_alig_frames = compute_num_frames(src_alig_frames[0], frame_length, source_sample_rate), compute_num_frames(src_alig_frames[1], frame_length, source_sample_rate)
            ...
            # if only target speech is available add target text ids and update the offset 
            text_ids = torch.as_tensor([tokenizer.bos] + tokenizer.text_to_ids(tgt_alignment[0]))
            tgt_alig_frames = [tgt_alignment[1], tgt_alignment[1]+tgt_alignment[2]]
            tgt_alig_frames = compute_num_frames(tgt_alig_frames[0], frame_length, target_sample_rate), compute_num_frames(tgt_alig_frames[1], frame_length, target_sample_rate)
            ...
            continue
        # 1) pad up to tgt_txt_start_pos if needed
        gap_to_text = tgt_txt_start_pos - cursor
        segments.append(_pad_chunk(gap_to_text, pad_id))
        cursor += max(0, gap_to_text)

        available_frames_for_text = tgt_end_pos - tgt_txt_start_pos + 1

         # 2) fit text within available frames [tgt_txt_start_pos .. tgt_end_pos]
        ...
        segments.append(text_ids)
        cursor += len(text_ids)
        eos_gap = tgt_end_pos - cursor + 1
        if eos_gap < 0:
            logging.warning(
                f"Truncating training example's text_ids of length {len(text_ids)} because {eos_gap=} < 0. {diagnostic}"
            )
            eos_gap = 0
        segments.append(_pad_chunk(eos_gap, pad_id))
        cursor += max(0, eos_gap)
        segments.append(torch.tensor([tokenizer.eos], dtype=torch.long))
        cursor += 1
    tokens = torch.cat(segments, dim=0)        
    return tokens
